...
crawler: ZhihuCrawler | None = None


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a requirement-driven Zhihu crawl.")
    ...
    parser.add_argument("--login-type", choices=["qrcode", "phone", "cookie"], help="Zhihu login type override.")
    parser.add_argument(
        "--save-option",
    ...
    parser.add_argument("--note-url", action="append", default=[], help="Zhihu note URL for detail mode. Repeatable.")
    parser.add_argument("--creator-url", action="append", default=[], help="Zhihu creator URL for creator mode. Repeatable.")
    ...
    parser.add_argument("--creator-contents-limit", type=int, default=20, help="Creator contents page size.")
...
def _load_requirement(args: argparse.Namespace) -> ZhihuCrawlRequirement:
    if args.file_path:
        payload = json.loads(Path(args.file_path).read_text(encoding="utf-8"))
        return ZhihuCrawlRequirement(**payload)
