...
class BilibiliPlatformHooks(BasePlatformHooks):
    ...
    def build_connector(self):
        return build_bilibili_connector_from_legacy(self.crawler)
    ...
    async def handle_success(self, *, task: CrawlTask, request: PlatformTaskRequest, task_result: PlatformTaskResult, job_id: str, task_id: str, services: ExecutionServices) -> dict[str, Any]:
        ...
        params = task.params or {}
        if task.task_type == "search":
            await services.raw_record_service.append(RawRecord(platform_code=self.platform_code, record_type="search", source_uri="/x/web-interface/wbi/search/type", request_meta={"keyword": params.get("keyword"), "page": params.get("page"), "job_id": job_id}, response_body=payload.get("raw"), metadata={"bridge": "bilibili_connector", "items_count": len(payload.get("items", []))}))
            await services.event_service.append(CrawlJobEvent(job_id=job_id, event_type="search_page_succeeded", message="Bilibili bridge search page succeeded", details={"keyword": params.get("keyword"), "page": params.get("page"), "items_count": len(payload.get("items", []))}), platform_code=self.platform_code)
            return payload
        if task.task_type == "detail":
            video = payload["video"]
            ...
            await services.raw_record_service.append(RawRecord(platform_code=self.platform_code, record_type="detail", source_uri=payload.get("request_uri", "/x/web-interface/view/detail"), request_meta={"content_id": params.get("content_id"), "bvid": params.get("bvid", ""), "job_id": job_id}, response_body=payload.get("raw_payload"), metadata={"bridge": "bilibili_connector"}))
            await services.event_service.append(CrawlJobEvent(job_id=job_id, event_type="detail_succeeded", message="Bilibili bridge detail succeeded", details={"content_id": params.get("content_id"), "bvid": params.get("bvid", "")}), platform_code=self.platform_code)
            return {"video": video}
        if task.task_type == "comments":
            await services.raw_record_service.append(RawRecord(platform_code=self.platform_code, record_type="comments", source_uri=payload.get("request_uri", "/x/v2/reply/wbi/main"), request_meta={"content_id": params.get("content_id"), "job_id": job_id}, response_body=payload.get("raw_payload"), metadata={"bridge": "bilibili_connector", "comment_count": len(payload.get("comments", []))}))
            await services.event_service.append(CrawlJobEvent(job_id=job_id, event_type="comments_succeeded", message="Bilibili bridge comments succeeded", details={"content_id": params.get("content_id"), "comment_count": len(payload.get("comments", []))}), platform_code=self.platform_code)
            return payload
        if task.task_type == "creator":
            await services.raw_record_service.append(RawRecord(platform_code=self.platform_code, record_type="creator", source_uri=payload.get("request_uri", "/x/space/wbi/acc/info"), request_meta={"creator_id": params.get("creator_id"), "job_id": job_id}, response_body=payload.get("raw_payload"), metadata={"bridge": "bilibili_connector"}))
            await services.event_service.append(CrawlJobEvent(job_id=job_id, event_type="creator_succeeded", message="Bilibili bridge creator succeeded", details={"creator_id": params.get("creator_id")}), platform_code=self.platform_code)
            return payload
        ...
        await services.raw_record_service.append(RawRecord(platform_code=self.platform_code, record_type="creator_contents", source_uri=payload.get("request_uri", "/x/space/wbi/arc/search"), request_meta={"creator_id": params.get("creator_id"), "job_id": job_id, "cursor": params.get("cursor", "")}, response_body=payload.get("raw_payload"), metadata={"bridge": "bilibili_connector", "normalized_count": len(normalized_records)}))
        await services.event_service.append(CrawlJobEvent(job_id=job_id, event_type="creator_contents_succeeded", message="Bilibili bridge creator contents succeeded", details={"creator_id": params.get("creator_id"), "items_count": len(video_items)}), platform_code=self.platform_code)
