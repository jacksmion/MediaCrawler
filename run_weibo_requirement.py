...
crawler: WeiboCrawler | None = None


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a requirement-driven Weibo crawl.")
    ...
    parser.add_argument("--login-type", choices=["qrcode", "phone", "cookie"], help="Weibo login type override.")
    parser.add_argument(
        "--save-option",
    ...
    parser.add_argument("--note-id", action="append", default=[], help="Weibo note id for detail mode. Repeatable.")
    parser.add_argument("--creator-id", action="append", default=[], help="Weibo creator id for creator mode. Repeatable.")
    ...
    parser.add_argument("--search-type", help="Weibo search type override.")
    ...
    parser.add_argument("--creator-contents-limit", type=int, default=10, help="Creator contents page size.")
...
def _load_requirement(args: argparse.Namespace) -> WeiboCrawlRequirement:
    if args.file_path:
        payload = json.loads(Path(args.file_path).read_text(encoding="utf-8"))
        return WeiboCrawlRequirement(**payload)
