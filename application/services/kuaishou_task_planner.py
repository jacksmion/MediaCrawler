...
class KuaishouTaskPlanner:
    """Build persisted crawl tasks from user-facing Kuaishou requirements."""

    def plan(self, requirement: KuaishouCrawlRequirement) -> list[CrawlTask]:
    ...
    def _plan_search(self, requirement: KuaishouCrawlRequirement) -> list[CrawlTask]:
        ...
        for keyword in keywords:
            for page in range(requirement.start_page, requirement.start_page + requirement.max_pages):
                tasks.append(
                    self._new_task(
                        task_type="search",
                        params={"keyword": keyword, "page": page, "page_size": requirement.page_size, "search_session_id": ""},
                    )
                )
        return tasks

    def _plan_detail(self, requirement: KuaishouCrawlRequirement) -> list[CrawlTask]:
    ...
    def _plan_creator(self, requirement: KuaishouCrawlRequirement) -> list[CrawlTask]:
        tasks: list[CrawlTask] = []
    ...
    def _new_task(*, task_type: str, params: dict) -> CrawlTask:
        return CrawlTask(
            task_id=f"ks-plan-{task_type}-{uuid.uuid4().hex[:12]}",
